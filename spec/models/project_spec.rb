...
RSpec.describe Project, type: :model do
	...
	it "is invalid without the Description." do
		project = build(:new_project, description: nil) # Cria mas não salva
		project.valid?
		expect(project.errors[:description]).to end_with("não pode ficar em branco") # I18n português
	end

	it "It is invalid without the start date." do
		project = build(:new_project, start_date: nil)
		project.valid?
		expect(project.errors[:start_date]).to end_with("não pode ficar em branco") # I18n português
	end

	it "It is invalid without the end date" do
		project = build(:new_project, end_date: nil)
		project.valid?
		expect(project.errors[:end_date]).to end_with("não pode ficar em branco") # I18n português
	end
